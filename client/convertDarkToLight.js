...
function processFile(path) {
    ...
    // careful replacing text-white, only whole words
    text = text.replace(/ text-white /g, ' text-slate-900 ');
    text = text.replace(/text-white\b/g, 'text-slate-900');
    ...
    text = text.replace(/text-slate-900\/(\d+)/g, 'text-slate-'); // heuristic clear up
    
    fs.writeFileSync(path, text, 'utf8');
}

processFile('src/site-engg/components/dashboards/HRDashboard.tsx');
processFile('src/site-engg/components/dashboards/AdminDashboard.tsx');
