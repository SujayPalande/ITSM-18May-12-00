...
import { LogOut, User, Home, HardHat, ChevronRight, Zap } from 'lucide-react';
...
export default function Header({ currentRole, userName, onProfileClick }: HeaderProps) {
  ...
  const roleConfig: Record<string, { label: string; gradient: string; dot: string; badge: string; accentBar: string }> = {
    admin:    { label: 'Administrator',  gradient: 'from-violet-600 to-purple-700',  dot: 'bg-violet-400', badge: 'bg-violet-500/10 text-violet-400 border-violet-500/20 ring-1 ring-violet-500/10', accentBar: 'from-violet-600 via-blue-500 to-violet-600' },
    engineer: { label: 'Field Engineer', gradient: 'from-blue-600 to-indigo-700',    dot: 'bg-blue-400',   badge: 'bg-blue-500/10 text-blue-400 border-blue-500/20 ring-1 ring-blue-500/10',         accentBar: 'from-blue-600 via-indigo-500 to-blue-600' },
    hr:       { label: 'HR Manager',     gradient: 'from-emerald-600 to-teal-700',   dot: 'bg-emerald-400', badge: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 ring-1 ring-emerald-500/10', accentBar: 'from-emerald-600 via-teal-500 to-emerald-600' },
    client:   { label: 'Client Portal',  gradient: 'from-amber-500 to-orange-600',   dot: 'bg-amber-400',  badge: 'bg-amber-500/10 text-amber-400 border-amber-500/20 ring-1 ring-amber-500/10',     accentBar: 'from-amber-500 via-orange-400 to-amber-500' },
  };
  ...
  return (
    <header className="bg-[#0d1117] border-b border-white/[0.06] sticky top-0 z-50 h-14 flex flex-col">
      {/* Brand accent line synced to role + logo palette */}
      <div className={`h-0.5 w-full bg-gradient-to-r ${cfg.accentBar} opacity-80`} />

      <div className="flex-1 flex items-center w-full max-w-[1440px] mx-auto px-5 lg:px-6 justify-between gap-4">

        {/* Brand */}
        <div className="flex items-center gap-3 min-w-0">
          {/* Logo mark — matches Cybaem Tech circular CT motif */}
