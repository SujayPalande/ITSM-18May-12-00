...
export default function ClientDashboard() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#f4f6f9] flex items-center justify-center">
      <div className="text-center space-y-4">
        <div className="w-10 h-10 border-2 border-slate-200 border-t-amber-500 rounded-full animate-spin mx-auto" />
  ...
  const statBlocks = [
    { label: 'Engineers',       value: assignments.length, icon: Users,    gradient: 'from-amber-500 to-orange-500',  bg: 'bg-amber-50',   iconColor: 'text-amber-600' },
    ...
    { label: 'Check-ins Today', value: checkIns.length,    icon: MapPin,   gradient: 'from-blue-500 to-indigo-600',   bg: 'bg-blue-50',    iconColor: 'text-blue-600' },
    { label: 'On Leave',        value: onLeaveCount,       icon: Calendar, gradient: 'from-red-500 to-rose-600',      bg: 'bg-red-50',     iconColor: 'text-red-500' },
  ];
    ...
    <div className="flex min-h-screen bg-[#f4f6f9]">
        ...
        <div className="px-5 pt-6 pb-5 border-b border-white/[0.06]">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-white/30 text-[11px] font-medium">Client Portal</p>
            </div>
          </div>
      ...
      <main className="ml-64 flex-1 min-h-screen flex flex-col">
        ...
        <div className="flex-1 p-8 space-y-6">
          ...
          {tab === 'overview' && (
            <div>
              <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-4">Assigned Engineers</p>
              <div className="grid gap-3 md:grid-cols-2">
                {assignments.length > 0 ? assignments.map(a => {
                  ...
                  return (
                    <div key={a.id} className="flex items-center justify-between gap-4 p-5 rounded-2xl bg-white border border-slate-100 hover:border-amber-200 hover:shadow-md transition-all duration-150">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-bold text-lg shadow-md shadow-amber-200 shrink-0">
                          {eng?.name?.charAt(0) || '?'}
                        </div>
                        <div>
                  ...
                  <div className="col-span-2 py-20 rounded-2xl border border-dashed border-slate-200 bg-white text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-slate-500 text-sm font-semibold">No engineers assigned</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          {tab === 'enterprise' && (
            <div className="space-y-4">
              <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest">Daily Attendance</p>
                ...
                {checkIns.length > 0 ? checkIns.map(ci => {
                  ...
                  return (
                    <div key={ci.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-2xl bg-white border border-slate-100 hover:border-amber-200 hover:shadow-md transition-all duration-150">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border shadow-sm ${ci.checkOutTime ? 'bg-slate-100 border-slate-200' : 'bg-emerald-50 border-emerald-200'}`}>
                          <Clock className={`w-5 h-5 ${ci.checkOutTime ? 'text-slate-400' : 'text-emerald-600'}`} />
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-sm">{eng?.name || 'Unknown'}</p>
                          <div className="flex flex-wrap items-center gap-2 mt-1.5">
                            <span className="text-[11px] font-bold text-slate-600 bg-slate-100 px-2.5 py-0.5 rounded-lg border border-slate-200">IN: {new Date(ci.checkInTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                            {ci.checkOutTime ? (
                              ...
                              <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded-full uppercase tracking-wider">On Site</span>
                            )}
                          </div>
                        </div>
                      </div>
                      {ci.latitude && ci.longitude && (
                        <a href={`https://www.google.com/maps?q=${ci.latitude},${ci.longitude}`} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-2 text-amber-700 text-xs font-bold bg-amber-50 hover:bg-amber-100 px-4 py-2.5 rounded-xl transition-colors shrink-0 border border-amber-200 shadow-sm">
          ...
          {tab === 'reports' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest">Work Reports</p>
                    ...
                    <div key={r.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:border-amber-200 hover:shadow-md transition-all duration-150">
                      <div className="flex items-center gap-4 px-6 py-4 border-b border-slate-50 bg-slate-50/50">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {site && <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-lg border border-slate-200">{site.name}</span>}
                        </div>
                        <span className="text-slate-400 text-xs shrink-0 font-medium bg-slate-100 px-2.5 py-1 rounded-full">{new Date(r.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
