...
export default function EngineerDashboard() {
  ...
  const statBlocks = [
    { label: 'Duty Status',    value: isActive ? 'On Duty' : isDone ? 'Complete' : 'Off Duty', icon: Activity,  color: 'text-blue-600',   bg: 'bg-blue-50',   border: 'border-blue-100' },
    { label: 'Sites Assigned', value: assignments.length,                                        icon: Briefcase, color: 'text-violet-600', bg: 'bg-violet-50', border: 'border-violet-100' },
    { label: 'My Reports',     value: reports.length,                                            icon: FileText,  color: 'text-emerald-600',bg: 'bg-emerald-50',border: 'border-emerald-100' },
    { label: 'Leave Requests', value: leaves.length,                                             icon: Calendar,  color: 'text-amber-600',  bg: 'bg-amber-50',  border: 'border-amber-100' },
  ];
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <aside className="w-60 shrink-0 fixed top-14 left-0 h-[calc(100vh-3.5rem)] bg-white border-r border-slate-200 flex flex-col z-30">
        ...
        <div className="px-4 pt-5 pb-4 border-b border-slate-100">
          ...
          <div className={`mt-3 flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold ${isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-50 text-slate-500'}`}>
      ...
      <main className="ml-60 flex-1 min-h-screen flex flex-col">
        ...
        <div className="flex-1 p-8 space-y-6">
          ...
          {/* ── SHIFT CONTROL CARD ── */}
          <div className={`rounded-xl border bg-white p-6 flex items-center justify-between gap-6 ${
            isActive ? 'border-emerald-200' : isDone ? 'border-slate-200' : 'border-slate-200'
          }`}>
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                isActive ? 'bg-emerald-100' : isDone ? 'bg-slate-100' : 'bg-blue-100'
              }`}>
                {isActive
            ...
            <div className="shrink-0">
              {!todayCheckIn ? (
                <button onClick={checkIn} disabled={sub}
                  className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold text-sm transition-all disabled:opacity-50">
                ...
                <button onClick={checkOut} disabled={sub}
                  className="flex items-center gap-2 px-6 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-lg font-semibold text-sm transition-all disabled:opacity-50">
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-slate-700 text-sm font-semibold">Shift History</p>
                </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {checkIns.length > 0 ? checkIns.slice(0, 20).map(ci => (
                      <tr key={ci.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-5 py-3.5 text-slate-400 text-xs truncate max-w-[180px]">
                          {ci.locationName ? (
                      ...
                      <tr>
                        <td colSpan={5} className="py-16 text-center">
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <p className="text-slate-400 text-xs">Start your first shift using the button above</p>
                          </div>
                        </td>
                      </tr>
                    )}
              ...
              <div className="lg:col-span-2">
                ...
                <form onSubmit={submitReport} className="space-y-4 bg-white rounded-xl border border-slate-200 p-5">
                  ...
                  <div>
                    ...
                    <textarea value={repForm.issues} onChange={e => setRepForm({ ...repForm, issues: e.target.value })} className={`${F} min-h-[68px]`} placeholder="Any blockers or issues?" />
                  </div>
                  <button type="submit" disabled={sub}
                    className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              <div className="lg:col-span-3">
                ...
                <div className="space-y-3">
                  {reports.length > 0 ? reports.sort((a, b) => new Date(b.date || b.createdAt).getTime() - new Date(a.date || a.createdAt).getTime()).map(r => (
                    <div key={r.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden hover:border-blue-200 hover:shadow-sm transition-all">
                    ...
                    <div className="py-16 rounded-xl border border-dashed border-slate-200 bg-white text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-slate-400 text-xs">Submit your first daily report</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="lg:col-span-3">
                ...
                <div className="space-y-3">
                  {leaves.length > 0 ? leaves.map(l => (
                    <div key={l.id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between hover:border-blue-200 hover:shadow-sm transition-all">
                      <div className="flex items-center gap-3">
                    ...
                    <div className="py-16 rounded-xl border border-dashed border-slate-200 bg-white text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-slate-400 text-xs">Submit a request using the form</p>
                      </div>
                    </div>
                  )}
                </div>
