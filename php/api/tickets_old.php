...
function handleCreateTicket($request) {
    try {
        ...
        if (!empty($_FILES['attachment']) && $_FILES['attachment']['error'] === UPLOAD_ERR_OK) {
            ...
            $fn = time() . '_' . $_FILES['attachment']['name'];
...
function handleUpdateTicket($id, $request) {
    ...
    try {
        ...
        $ticket = $db->fetchOne("SELECT title, created_by_id, assigned_to_id FROM tickets WHERE id = ?", [$id]);
        if ($ticket) {
            $url = "/tickets/$id";
            // If status changed
            ...
            if (isset($request['assignedToId']) && $request['assignedToId']) {
